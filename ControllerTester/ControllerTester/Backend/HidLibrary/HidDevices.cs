...
namespace HidLibrary {
    ...
    public class HidDevices {
        ...
        public static bool IsConnected(string devicePath) { return EnumerateDevices().Any(x => x.Path == devicePath); }

        public static HidDevice GetDevice(string devicePath) { return Enumerate(devicePath).FirstOrDefault(); }
        ...
        public static IEnumerable<HidDevice> Enumerate(int vendorId, params int[] productIds) {
            return
                EnumerateDevices().Select(x => new HidDevice(x.Path, x.Description)).Where(
                                                                                           x =>
                                                                                           x.Attributes.VendorId ==
                                                                                           vendorId &&
                                                                                           productIds.Contains(
                                                                                                               x
                                                                                                                   .Attributes
        ...
        private static string GetDeviceDescription(IntPtr deviceInfoSet, ref NativeMethods.SpDevinfoData devinfoData) {
            ...
            NativeMethods.SetupDiGetDeviceRegistryProperty(
                                                           ...
                                                           NativeMethods.SpdrpDevicedesc, ref type, descriptionBuffer,
                                                           descriptionBuffer.Length, ref requiredSize);

            return descriptionBuffer.ToUtf8String();
        }

        private static string GetBusReportedDeviceDescription(
            IntPtr deviceInfoSet, ref NativeMethods.SpDevinfoData devinfoData) {
            var descriptionBuffer = new byte[1024];
            ...
            if(Environment.OSVersion.Version.Major > 5) {
                ...
                var _continue = NativeMethods.SetupDiGetDeviceProperty(
                                                                       ...
                                                                       ref
                                                                           NativeMethods
                                                                           .DevpkeyDeviceBusReportedDeviceDesc,
                                                                       ref propertyType, descriptionBuffer,
                                                                       descriptionBuffer.Length, ref requiredSize, 0);
